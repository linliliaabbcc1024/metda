angular
.module('blankapp').controller("sum_normalizationController", function($scope, $rootScope, $timeout, $mdToast){
  ctrl = this;
  ...
  ctrl.submit = function(){
    ...
    var req = ocpu.call("sum_normalization_fun",ctrl.parameters,function(session){
      sss = session
      session.getObject(function(obj){
        oo = obj
  ...
  ctrl.download = function(){
    ...
    .then(function (blob) {
      saveAs(blob, "Sum Normalization - Plots.zip");
    });*/

      download_csv(Papa.unparse(oo.data_matrix), "Sum Normalization "+time_stamp+".csv")
  }
  ...
  ctrl.save_result = function(){
    // trying to save result. The result must be in a form of [{},{},{}], which is a folder of the tree. In one of the {}, there is a main key indicating that this is the folder node. If the main is not found, then everything will be added to the user clicked node. For all the nodes that are not folder node, must have 'saving_content' and 'content_type' for adding the attachments. Also, these nodes's parent is to be determined by the user click.
        ...
        var to_be_saved =
        [{
          "id":"sum_normalization_dataset_"+time_stamp,
          "parent":undefined,
          "text":"Sum Normalization",
          "icon":"fa fa-folder",
          "main":true
        },{
          "id":"sum_normalization_dataset_"+time_stamp+".csv",
          "parent":"sum_normalization_dataset_"+time_stamp,
          "text":"Sum Normalization.csv",
          "icon":"fa fa-file-excel-o",
          "attachment_id":"sum_normalization_dataset_"+time_stamp+".csv",
          "saving_content":btoa(Papa.unparse(oo.data_matrix)),
