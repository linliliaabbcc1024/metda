angular
    .module('blankapp').controller("two_way_anovaController", function($scope, $rootScope, Upload, $timeout, $mdToast){ ctrl = this;
  ...
  ctrl.submit = function(){
    if(ctrl.parameters.columns.length!==2){
      alert("You have to select two group column. Instead you selected "+ctrl.parameters.columns.length+" columns ("+ctrl.parameters.columns.join(", ")+").")
      return;
    }

    ctrl.submit_button_text = "Calculating"

    var req = ocpu.call("two_way_anova_fun",ctrl.parameters,function(session){
      sss = session
      session.getObject(function(obj){
        oo = obj
  ...
  ctrl.download = function(){
    ...
    .then(function (blob) {
      saveAs(blob, "Two-way ANOVA - Plots.zip");
    });*/

      download_csv(Papa.unparse(oo.result), "Two-way ANOVA"+time_stamp+".csv")
  }
  ...
  ctrl.save_result = function(){
    ...
    var to_be_saved =
    [{
          "id":"two_way_anova_result_"+time_stamp,
          "parent":undefined,
          "text":"Two-way ANOVA",
          "icon":"fa fa-folder",
          "main":true,
    ...
    "id":"two_way_anova_result_"+time_stamp+".csv",
    "parent":"two_way_anova_result_"+time_stamp,
