angular
.module('blankapp').controller("two_way_mixed_anovaController", function($scope, $rootScope, Upload, $timeout, $mdToast){ ctrl = this;
...
ctrl.make_data_read_here = function(obj){
  ...
  ctrl.parameters.groups_ind = ctrl.column_options.includes(parameters.column)?parameters.column:ctrl.column_options[0]
...
ctrl.submit = function(){
  ...
  var req = ocpu.call("two_way_mixed_anova_fun",ctrl.parameters,function(session){
    sss = session
    session.getObject(function(obj){
      oo = obj
...
ctrl.download = function(){
  ...
  .then(function (blob) {
    saveAs(blob, "Two-way Mixed ANOVA - Plots.zip");
  });*/

    download_csv(Papa.unparse(oo.result), "Two-way Mixed ANOVA"+time_stamp+".csv")
}
...
ctrl.save_result = function(){
  // trying to save result. The result must be in a form of [{},{},{}], which is a folder of the tree. In one of the {}, there is a main key indicating that this is the folder node. If the main is not found, then everything will be added to the user clicked node. For all the nodes that are not folder node, must have 'saving_content' and 'content_type' for adding the attachments. Also, these nodes's parent is to be determined by the user click.
    ...
    var to_be_saved =
    [{
          "id":"two_way_mixed_anova_result_"+time_stamp,
          "parent":undefined,
          "text":"Two-way Mixed ANOVA",
          "icon":"fa fa-folder",
          "main":true,
          "analysis_type":"two_way_mixed_anova",
          "parameters":to_be_saved_parameters
        },{
    "id":"two_way_mixed_anova_result_"+time_stamp+".csv",
    "parent":"two_way_mixed_anova_result_"+time_stamp,
    "text":"Two-way Mixed ANOVA result.csv",
    "icon":"fa fa-file-excel-o",
    "attachment_id":"two_way_mixed_anova_dataset_"+time_stamp+".csv",
