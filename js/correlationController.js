angular
    .module('blankapp').controller("correlationController", function($scope, $rootScope, $timeout, $mdToast){
      ctrl = this;
      ...
      ctrl.make_data_read_here2 = function(obj){
        ...
        ctrl.parameters.method = parameters.correlation.method
        $scope.$watch("ctrl.parameters",function(newValue, oldValue){
          parameters.correlation.method = ctrl.parameters.method
      ...
      ctrl.submit = function(){
        ...
        var req = ocpu.call("correlation_fun",ctrl.parameters,function(session){
          sss = session
          session.getObject(function(obj){
      ...
      ctrl.download = function(){
        ...
        .then(function (blob) {
            saveAs(blob, "Correlations - Plots.zip");
        });*/

        var zip = new JSZip();
        zip.file("Correlations "+time_stamp+".csv", Papa.unparse(oo.data_matrix))
        zip.file("Correlations "+time_stamp+".csv", Papa.unparse(oo.data_matrix2))
        zip.generateAsync({type:"blob"})
        .then(function (blob) {
            saveAs(blob, "Correlation Analysis.zip");
        });
    ...
    to_be_saved_parameters.p = null
        var to_be_saved =
        [{
          "id":"correlation_dataset_"+time_stamp,
          "parent":undefined,
          "text":"Correlations Analysis",
          "icon":"fa fa-folder",
          "main":true,
          "analysis_type":"correlation",
          "parameters":to_be_saved_parameters
        },{
          "id":"correlation_dataset_"+time_stamp+".csv",
          "parent":"correlation_dataset_"+time_stamp,
          "text":"Correlation Coefficient.csv",
          "icon":"fa fa-file-excel-o",
          "attachment_id":"correlation_dataset_"+time_stamp+".csv",
          "saving_content":btoa(unescape(encodeURIComponent(Papa.unparse(oo.data_matrix)))),
          "content_type":"application/vnd.ms-excel"
        },{
          "id":"correlation_dataset2_"+time_stamp+".csv",
          "parent":"correlation_dataset_"+time_stamp,
          "text":"Correlation p values.csv",
          "icon":"fa fa-file-excel-o",
          "attachment_id":"correlation_dataset2_"+time_stamp+".csv",
