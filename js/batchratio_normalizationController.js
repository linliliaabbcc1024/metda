angular
.module('blankapp').controller("batchratio_normalizationController", function($scope, $rootScope, $timeout, $mdToast,cfpLoadingBar){
  ctrl = this;
  ...
  ctrl.make_data_read_here = function(obj){
    ...
    ctrl.parameters.batch_column = ctrl.column_options.includes(parameters.batchratio.batch_column)?parameters.batchratio.batch_column:ctrl.column_options[0]
    ctrl.parameters.time_column = ctrl.column_options.includes(parameters.batchratio.time_column)?parameters.batchratio.time_column:ctrl.column_options[0]
    ctrl.parameters.qc_column = ctrl.column_options.includes(parameters.batchratio.qc_column)?parameters.batchratio.qc_column:ctrl.column_options[0]
    ...
    $scope.$watch("ctrl.parameters.qc_column",function(newValue, oldValue){
      ...
      ctrl.parameters.qc_level = ctrl.qc_level_options.includes(parameters.batchratio.qc_level)?parameters.batchratio.qc_level:[ctrl.qc_level_options[0]]
    },true)
    $scope.$watch("ctrl.parameters",function(newValue, oldValue){
      parameters.batchratio.qc_column = ctrl.parameters.qc_column
      parameters.batchratio.batchratio_column = ctrl.parameters.batchratio_column
      parameters.batchratio.time_column = ctrl.parameters.time_column
      parameters.batchratio.qc_level = ctrl.parameters.qc_level
  ...
  ctrl.submit = function(){
    ...
    var req = ocpu.call("batchratio_normalization_fun",ctrl.parameters,function(session){
      sss = session
      session.getObject(function(obj){
        oo = obj
  ...
  ctrl.download = function(){
    ...
    .then(function (blob) {
      saveAs(blob, "Batch Ratio Normalization - Plots.zip");
    });*/

      download_csv(Papa.unparse(oo.data_matrix), "Batch Ratio Normalization "+time_stamp+".csv")
  }
  ...
  ctrl.save_result = function(){
    ...
    to_be_saved_parameters.p = null
        var to_be_saved =
        [{
          "id":"batchratio_normalization_dataset_"+time_stamp,
          "parent":undefined,
          "text":"Batch Ratio Normalization",
          "icon":"fa fa-folder",
          "main":true,
          "analysis_type":"batchratio_normalization",
          "parameters":to_be_saved_parameters
        },{
          "id":"batchratio_normalization_dataset_"+time_stamp+".csv",
          "parent":"batchratio_normalization_dataset_"+time_stamp,
          "text":"Batch Ratio Normalization.csv",
          "icon":"fa fa-file-excel-o",
          "attachment_id":"batchratio_normalization_dataset_"+time_stamp+".csv",
