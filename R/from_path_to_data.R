from_path_to_data = function(path){
...
# e = data.matrix(fread(paste0("http://slfan:metabolomics@localhost:5985/project/",project_id,"/temp_e.csv")))
  ...
  if(ncol(e)>2){
    ...
    if(!is.null(dim(f))){
      ...
      f = data.table(label = f[!bad_compounds_with_zero_sd])
    }
  ...
  return(list(e = e, p = p, f = f, data_matrix = data_matrix))
}
